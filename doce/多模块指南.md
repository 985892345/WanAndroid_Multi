...
2. 修改 `build.gradle`，由于项目中自定义了模块的管理（`build_logic`），所以使用 AS 新建模块需要修改一些东西
   ...
   ```kotlin
   plugins {
       id("manager.library") // 该插件整合了所有模板配置
   }
   
   useARouter() // 如果需要使用 ARouter
   
   dependencies {
       implementation(projects.libBase)       // 依赖 base 模块
       implementation(projects.libUtils)      // 依赖 utils 模块
       implementation(projects.libConfig)     // 依赖 config 模块
       implementation(projects.libAccount.apiAccount)   // 依赖 account 模块的 api
       
       // lib 模块一般依赖 projectBase 依赖整合包即可，当然你也可以依赖其他的，建议按需依赖
       // 其他依赖可以 Ctrl 点击 projectBase 跳转后查看
...
2. 创建好了后需要与[创建 `lib` 模块](#新建lib模块)一样修改一遍，这里不再叙述
   ...
   >    
   >    // module 模块一般依赖下面三个依赖包即可
   >    // 如果还有其他依赖如 glide、lottie 等，请具体进行依赖
   >    implementation(libs.bundles.projectBase)
...
9. 由于 AS 的原因，每次按照它的模板新建 `Activity` 和 `Fragment`，它都会主动添加 `android` 插件和默认依赖到 `build.gradle.kts` 中，如下：
   ...
   > 注：id("module-manager") 是之前的插件名字，目前已经修改为 id("manager.library")
...
4. 在需要得到该接口的模块中引入该 `api` 模块
   ...
   // 则在 module_main 的 build.gradle.kts 中依赖 api_test：
   dependencies {
       implementation(projects.moduleTest.apiTest)
   }
   ```
...
1. 在 `module_test` 的 `build.gradle.ktx` 中修改插件如下
   ...
   // 现目前支持判断什么时候使用单模块调试功能，不用再改回 manager.library 插件
   ```
...
这个大概率是存在了两个都带有 `application` 插件的模块之间互相有依赖，目前单模块调试有自动取消的功能，
但是是以 gradle 当前执行的 task 名字来进行判断的，可以会有遗漏

可以在 `build_logic/manager` 下的 `manager.libraryApp.gradle.kts` 中添加漏掉的 gradle task name
